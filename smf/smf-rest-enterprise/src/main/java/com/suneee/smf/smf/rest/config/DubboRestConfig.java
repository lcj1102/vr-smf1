package com.suneee.smf.smf.rest.config;
...
import com.suneee.resteasy.constants.DubboConfig;
...
@Configuration
@ComponentScan("com.suneee.smf.smf")//配置扫描包，需要包含spring配置类所在包，可以使用@ComponentScan({"com.suneee.scn.sys.rest.impl","com.suneee.scn.sys.config"})
public class DubboRestConfig {
    private static final String APPLICATION_NAME;//rest提供者名称
    private static final String APPLICATION_OWNER;//rest提供者所有者
    private static final String REGISTRY_ADDRESS;//注册dubbo服务的地址
    private static final String DUBBO_SERVER;//dubbo使用的协议的服务器端实现类型
    private static final String REST_FILTER;//拦截器
    private static final int   REST_TIMEOUT;//超时时间
    private static final String PROTOCOL_SERIALIZATION;//dubbo协议序列化方式
    private static final int   PROTOCOL_PORT;//rest提供者的接口,web调用时候使用
    private static final String PROTOCOL_NAME;//dubbo协议名称
    private static final String PROTOCOL_EXTENSION = DubboConfig.DUBBO_PROTOCOL_EXTENSION;
    public static final String ANNOTATION_PACKAGE = "com.suneee.resteasy.impl,com.suneee.smf.smf.rest.impl,com.suneee.smf.smf.consumer.impl";//提供者实现类的包路径，实现类必须添加dubbo的service注解

    static {
        //PropertiesFactory.createProperties第一个属性为该项目在配置中心的标识，第二个属性为配置中心对应文件夹下的配置文件名
        Properties properties = PropertiesFactory.createProperties("smf/smf","restDubbo");
